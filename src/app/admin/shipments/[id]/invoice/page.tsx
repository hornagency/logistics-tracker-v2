...
function invoiceNumber(id: string, createdAt: Date) {
  const date = createdAt.toISOString().slice(0, 10).replace(/-/g, "");
  return `VLA-INV-${date}-${id.slice(-6).toUpperCase()}`;
}
...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const invNo    = invoiceNumber(shipment.id, shipment.createdAt);
      ...
      <div className="print:hidden border-b border-slate-200 bg-white px-6 py-4">
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href={`/admin/shipments/${id}/receipt`} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">
              Switch to Receipt
            </Link>
      ...
      <div className="mx-auto max-w-4xl bg-white p-10 shadow-lg print:shadow-none print:p-0 print:max-w-none my-8 print:my-0">
        ...
        <div className="mt-8 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Document Type: Shipment Invoice</p>
          </div>
          <div className="text-right text-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Invoice No.</p>
            <p className="mt-0.5 text-base font-bold text-slate-900">{invNo}</p>
            <p className="mt-2 text-xs font-medium uppercase tracking-wide text-slate-400">Issue Date</p>
        ...
        <div className="mt-8 grid grid-cols-2 gap-8 rounded-xl bg-slate-50 p-6 print:bg-gray-50">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-primary">Sender / Shipper</p>
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-primary">Receiver / Consignee</p>
            <div className="mt-2 text-sm leading-6 text-slate-700">
        ...
        <div className="mt-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Shipment Details</p>
        ...
        {shipment.updates.length > 0 && (
          <div className="mt-8">
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Status Timeline</p>
            <table className="mt-3 w-full overflow-hidden rounded-xl border border-slate-200 text-sm">
        ...
        <div className="mt-12 border-t border-slate-200 pt-6 text-center text-xs text-slate-400">
          ...
          <p className="mt-1">This document is computer-generated and is valid without a signature.</p>
